...
export default function Landing() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    // If there's an exact match or single result, go directly to it
    if (filteredCelebrities.length > 0) {
      handleSuggestionClick(filteredCelebrities[0]);
    }
  };
    ...
    <div className="flex flex-col">
      ...
      <section className="relative flex items-center justify-center px-6 py-24 md:py-32 bg-gradient-to-br from-primary/5 via-background to-accent/5">
        <div className="max-w-5xl mx-auto text-center space-y-8 relative z-10">
          ...
          <div className="inline-block px-6 py-3 rounded-lg bg-green-500/10 border border-green-500/20">
            <p className="text-sm font-semibold text-green-600 dark:text-green-400">
              🎁 Get 20 FREE tokens when you sign up! (Worth ₹10)
            </p>
          </div>
      ...
      <section id="plans" className="px-6 py-16 md:py-24 bg-card/50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-5xl font-bold mb-4">Token-Based Pricing</h2>
            <p className="text-lg text-muted-foreground">Each generation costs 10 tokens • Buy tokens as you need them</p>
            <p className="text-sm text-green-600 dark:text-green-400 mt-2">New users get 20 FREE tokens to get started!</p>
          </div>

          <div className="grid md:grid-cols-4 gap-6">
            {[
              { name: "Starter", price: "₹5", tokens: "10", popular: false, desc: "Perfect for trying out" },
              { name: "Basic", price: "₹50", tokens: "100", popular: false, desc: "10 generations" },
              { name: "Pro", price: "₹100", tokens: "200", popular: true, desc: "20 generations" },
              { name: "Premium", price: "₹500", tokens: "1000", popular: false, desc: "100 generations" },
            ].map((plan) => (
              <Card 
                key={plan.name} 
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">{plan.tokens} tokens</p>
          ...
          <div className="mt-12 text-center">
            <Card className="inline-block p-6 bg-gradient-to-r from-primary/10 to-accent/10">
              <h3 className="text-xl font-semibold mb-2">Campaign Plan for Celebrities & Influencers</h3>
              <p className="text-3xl font-bold mb-2">Starting at ₹10,000</p>
              <p className="text-sm text-muted-foreground mb-2">25,000 tokens + custom campaign link</p>
              <p className="text-xs text-muted-foreground mb-4">Fans can generate photos for free using campaign tokens</p>
              <Button asChild data-testid="button-plan-campaign">
                <a href="mailto:fanai.partners@gmail.com">Contact Us</a>
              </Button>
            </Card>
          </div>
        </div>
