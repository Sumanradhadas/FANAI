...
export const users = pgTable("users", {
  ...
  tokens: integer("tokens").notNull().default(20), // Generation tokens (20 free tokens for new users)
...
export const templates = pgTable("templates", {
  ...
  campaignId: varchar("campaign_id").references(() => campaigns.id, { onDelete: 'cascade' }), // Exclusive to campaign if set
  isFree: boolean("is_free").notNull().default(false), // Free templates don't require tokens
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const campaigns = pgTable("campaigns", {
  ...
  celebrityId: varchar("celebrity_id").references(() => celebrities.id, { onDelete: 'set null' }),
  userId: varchar("user_id").references(() => users.id, { onDelete: 'cascade' }), // Campaign creator (nullable for admin-created campaigns)
  tokens: integer("tokens").notNull().default(0), // Campaign token pool (₹10,000 = 25,000 tokens)
  isActive: boolean("is_active").notNull().default(true),
...
// Plans table (token packages users can purchase)
export const plans = pgTable("plans", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: varchar("name", { length: 100 }).notNull(),
  slug: varchar("slug", { length: 100 }).notNull().unique(),
  price: integer("price").notNull(), // Price in rupees
  tokens: integer("tokens").notNull(), // Number of tokens (10 tokens = ₹5 for users)
