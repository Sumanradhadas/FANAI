...
export interface IStorage {
  ...
  deductUserTokens(userId: string, amount: number): Promise<void>;
  ...
  deductCampaignTokens(campaignId: string, amount: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deductUserTokens(userId: string, amount: number): Promise<void> {
    await db
      .update(users)
  ...
  async deductCampaignTokens(campaignId: string, amount: number): Promise<void> {
    await db
      .update(campaigns)
      .set({ tokens: sql`${campaigns.tokens} - ${amount}` })
