...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to map GitHub celebrity format to frontend format
  const mapCelebrityFormat = (celeb: any) => ({
    ...celeb,
    imageUrl: celeb.image, // Map 'image' to 'imageUrl' for frontend compatibility
  });

  app.get('/api/celebrities', async (req, res) => {
    try {
      const celebrities = await fetchCelebritiesFromGitHub();
      res.json(celebrities.map(mapCelebrityFormat));
    } catch (error) {
  ...
  app.get('/api/celebrities/search', async (req, res) => {
    try {
      ...
      res.json(results.map(mapCelebrityFormat));
    } catch (error) {
  ...
  app.get('/api/celebrities/:slug', async (req, res) => {
    try {
      ...
      res.json(mapCelebrityFormat(celebrity));
    } catch (error) {
  ...
  app.post('/api/generate', isAuthenticated, upload.single('photo'), async (req: any, res) => {
    try {
      ...
      // Check tokens (campaign tokens first, then user tokens)
      const user = await storage.getUser(userId);
      let campaign = null;
      const TOKENS_PER_GENERATION = 10;

      if (campaignId) {
        campaign = await storage.getCampaignById(campaignId);
      }

      // Check if user has enough tokens (campaign tokens can supplement)
      // If campaign exists, user only needs to cover the shortfall
      const campaignContribution = campaign ? Math.min(campaign.tokens, TOKENS_PER_GENERATION) : 0;
      const userTokensNeeded = TOKENS_PER_GENERATION - campaignContribution;
      
      if (!user || user.tokens < userTokensNeeded) {
      ...
      // Deduct tokens (10 tokens per generation)
      // Deduct from campaign first, then from user if campaign has insufficient tokens
      if (campaign && campaign.tokens >= TOKENS_PER_GENERATION) {
        // Campaign has enough tokens, use campaign tokens only
        await storage.deductCampaignTokens(campaignId, TOKENS_PER_GENERATION);
      } else if (campaign && campaign.tokens > 0) {
        // Campaign has some tokens but not enough
        // Deduct all remaining campaign tokens, then charge user the difference
        const campaignContribution = campaign.tokens;
        const userContribution = TOKENS_PER_GENERATION - campaignContribution;
        await storage.deductCampaignTokens(campaignId, campaignContribution);
        await storage.deductUserTokens(userId, userContribution);
      } else {
        // No campaign or campaign has no tokens, use user tokens only
        await storage.deductUserTokens(userId, TOKENS_PER_GENERATION);
      }

      // Increment campaign generation counter if applicable
      if (campaignId) {
  ...
  app.get('/api/campaigns/:slug/templates', async (req, res) => {
    try {
      const campaign = await storage.getCampaign(req.params.slug);
      if (!campaign) {
        return res.status(404).json({ message: "Campaign not found" });
      ...
      const campaignTemplates = allTemplates.filter(t => t.campaignId === campaign.id && t.isActive);
  ...
  app.post('/api/admin/campaigns', isAuthenticated, isAdmin, upload.array('templates'), async (req: any, res) => {
    try {
      // Admin creates campaigns without userId (set to null)
      const userId = req.session.isAdmin ? null : (req.userId || req.session.userId);
      ...
      const campaignData = {
        ...
        candidateName: req.body.candidateName || undefined,
        celebrityId: req.body.celebrityId || undefined,
        userId,
        tokens: parseInt(req.body.tokens) || 0,
        isActive: true,
      };

      // Validate and create campaign
      const validated = insertCampaignSchema.parse(campaignData);
      const campaign = await storage.createCampaign(validated);

      // Parse and create templates
      const templateData: any[] = [];
      const files = (req.files as Express.Multer.File[]) || [];
      
      // Extract template data from request body
      Object.keys(req.body).forEach(key => {
        const match = key.match(/^templates\[(\d+)\]\[(.+)\]$/);
        if (match) {
          ...
          templateData[index][field] = req.body[key];
        }
      });

      // Create templates with uploaded images
      let fileIndex = 0;
      for (const template of templateData) {
        if (template.name && template.prompt) {
          const templateSlug = template.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
          
          // Handle preview image upload if exists
          let previewUrl = undefined;
          if (files[fileIndex]) {
            const uploadedFile = files[fileIndex];
            const uploadPath = path.join('uploads', uploadedFile.filename);
            previewUrl = `/uploads/${uploadedFile.filename}`;
            fileIndex++;
          }

          const templateToCreate = {
            name: template.name,
            slug: `${campaign.slug}-${templateSlug}`,
            prompt: template.prompt,
            description: template.description || undefined,
            category: template.category || 'campaign',
            tags: template.tags ? template.tags.split(',').map((t: string) => t.trim()) : [],
            previewUrl,
          ...
          await storage.createTemplate(templateToCreate);
        }
      }
